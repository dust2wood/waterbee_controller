...
/*    if (color == YELLOW) {
    ...
    while (str[i] != 0) {
        if (str[i] > 0x2F && str[i] < 0x3A)
			...
			NEMERIC_18X23_BK_W(x,y,12,color);	// 없음 ,,,추가해야함
...
//=============================================
// IMG_set10_sensor 진단 메뉴
void display_set10_sensor(unsigned char select)
{
	if (select==0) 		NAND_ReadData2(	imageBuffer, 54,28,4);	// 정상
	else if (select==1)	NAND_ReadData2(	imageBuffer, 54,32,4);	// 이상, 센서 점검 
	else if (select==2)	NAND_ReadData2(	imageBuffer, 54,36,4);	// 이상, 램프 점검
	else 				NAND_ReadData2(	imageBuffer, 54,40,4);	// 센서 연결 실패
