...
/*				f1 = currentData.S1PPM/100.0;	// 예 100 = 1.00
				f2 = 8*f1 + 4;				 // y=8x+4
//				ppm_4_20ma = f2*100;
...
/*
				...
				for (i=1;i<18;i++) {
					if (ppm_4_20ma < PPM_4_20MA[i])
					{
						...
						imsi = (ppm_4_20ma-min)/(max-min);    // 비율을 구하고 

						min = PWM_4_20MA[i-1];
						max = PWM_4_20MA[i];
						trans_anlog_pwmVaule  = (max-min)*imsi + min; // 비율을 적용
					 	break;
					}
				}
*/				
  ...
  /*
				...
				//trans_anlog_pwmVaule = DAC_4_20MA[currentData.S2PPM/50]; // 200개일때
//				trans_anlog_pwmVaule = TABLE_4_20MA2[currentData.S2PPM/10];	// 1000개일때 (0~10000)
//				if (currentData.S2PPM>900) trans_anlog_pwmVaule+=40;
//				if (currentData.S2PPM>6000 && currentData.S2PPM<8100) trans_anlog_pwmVaule-=10;

				f1 = currentData.S2PPM/1000.0;	// 예 5000 =5.000
