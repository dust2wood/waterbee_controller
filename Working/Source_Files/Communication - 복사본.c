...
// RS232, RS485 시간지나면 4-20Ma 로 복귀하기
...
*/
	...
	if (state==0) 			ComCouter++;
    else if (state==0x15) 	ComCouter2++;


	// stste=0 은 메인메뉴이고, state=15 는 진단모드(diag) 일때의 통신 에러체크이다.
	// 메인메뉴에서는 빠르게 움직이므로 4000으로 했다. 약 4~5초
	// 진단메뉴에서는 느리게 움직이므로 70으로 했다. 약 4~5초
...
//        if (ComCouter_Prev == ComCouter) {
            if (currentData.Device_Selector_Mode & SENSOR_1_MODE) {
                ...
                currentData.S2PPM = 15;	// 최소값 0.015 
                currentData.temperature = 0;
                currentData.S2mV = 0;
                trans_anlog_pwmVaule = 0;
                trans_temp_pwmVaule = 0;
    ...
    if (currentData.Device_Selector_Mode & SENSOR_1_MODE) {
        ...
        if (configData.adjustConfig.filter_update_flag == 0) {
            ...
            if (first_flag == 1) {
                ...
                Adj_gradientS2 = (currentData.comm_S2NTU * 0.001) * (configData.adjustConfig.gradientS1 * 0.01);
                Adj_offsetS2 = (configData.adjustConfig.offsetS1 * 0.01);
            }
        }
        if (first_flag == 0 && configData.adjustConfig.filter_update_flag != 0) {
...
#ifndef SENSOR_PH_EC
		if (currentData.S2PPM <15) currentData.S2PPM=15;	// 최소값 0.015 탁도 
#else
...
void SendCalData000(uint8_t name, uint32_t vaule) {
...
void SendCalData(uint32_t value) {
	SendCalData000('D', value);
}

void SendCalDataCurrent(uint32_t value) {
	SendCalData000('C', value);
}

void SendCalZeroData(uint32_t value) {
	SendCalData000('B', value);
}

void SendCalZeroDataCurrent(uint32_t value) {
	SendCalData000('A', value);
}

void SendCalDataTemp(uint32_t value) {
	SendCalData000('T', value);
}
