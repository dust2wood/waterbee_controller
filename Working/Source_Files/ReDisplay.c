...
*/
		...
		case STATE_ALARM:
            ...
            if (currentData.Device_Selector_Mode & SENSOR_1_MODE) {
                ...
                sprintf(strBuffer, " %d.%d", tempConfigData.alarmConfig.highLimit2 / 1000, (tempConfigData.alarmConfig.highLimit2 / 100) % 10);
                //DrawMediumNumber(L_NUMBER1_X, L_NUMBER1_Y, strBuffer, YELLOW);
				if (tempConfigData.alarmConfig.highLimit2>=10000) 	DrawSmallNumber(43-12,180, strBuffer, YELLOW);
                else 											DrawSmallNumber(43,   180, strBuffer, YELLOW);
                //DrawLineRectangle(180, 83, 350, 123, BROWN);

                sprintf(strBuffer, "%02d", tempConfigData.alarmConfig.highLimit2 % 100);
                //DrawMediumNumber(L_NUMBER1_X + (22 * 3), L_NUMBER1_Y, strBuffer, BROWN);
                DrawSmallNumber(56+(12*3),180, strBuffer, BROWN);

                sprintf(strBuffer, " %d.%03d", tempConfigData.alarmConfig.lowLimit2 / 1000, tempConfigData.alarmConfig.lowLimit2 % 1000);
                //DrawMediumNumber(L_NUMBER1_X, L_NUMBER4_Y, strBuffer, BROWN);
				if (tempConfigData.alarmConfig.lowLimit2>=10000) 	DrawSmallNumber(195-12,180, strBuffer, BROWN);
                else 											DrawSmallNumber(195,   180, strBuffer, BROWN);
...
//				display_mV_1();
	            ...
	            // 센서 정상 이상, 추가
	            if (Sensor_State1 == SENSOR_OK) display_set10_sensor(0); // 정상 
	            else if (Sensor_State1 == SENSOR_ERROR1) display_set10_sensor(1); // 이상, 센서 점검 
	            else if (Sensor_State1 == SENSOR_ERROR2) display_set10_sensor(2); // 이상, 램프 점검
	            else display_set10_sensor(3); // 센서 연결 실패
            }
            else if (currentData.Device_Selector_Mode == SENSOR_2_MODE) {
...
//				display_CL2();
	            ...
	            // 센서 정상 이상, 추가
	            if (Sensor_State2 == SENSOR_OK) display_set10_sensor(0); // 정상 
	            else if (Sensor_State2 == SENSOR_ERROR1) display_set10_sensor(1); // 이상, 센서 점검 
	            else if (Sensor_State2 == SENSOR_ERROR2) display_set10_sensor(2); // 이상, 램프 점검
	            else display_set10_sensor(3); // 센서 연결 실패
            }
...
//                sprintf(strBuffer, "%1d.%02d", configData.outputConfig.output20mA / 1000, configData.outputConfig.output20mA % 1000);
                sprintf(strBuffer, "2.00");
	            DrawSmallNumber(100, 217, strBuffer, BROWN);

	            //		DrawLineRectangle(210, 113, 370, 153, BROWN);
	
	            //		DrawTextsize96(260, 126, TEXT96_4MA, DRAW_IMAGE_ENABLE);
	            //		Draw_Back_Oval4(260,144);  
	            ...
	            //		DrawTextsize96(260, 190, TEXT96_20MA, DRAW_IMAGE_ENABLE);
	            //		Draw_Back_Oval4(260,208);  
	            ...
	            DrawSmallNumber(90, 152, strBuffer, YELLOW);
...
//                sprintf(strBuffer, "%02d.%03d", configData.outputConfig.output20mA2 / 1000, configData.outputConfig.output20mA2 % 1000);
        ...
        case STATE_CALIB_TEMP:
            ...
            //DrawTextsize180(50, 120, TEXT180_S1_TEMP_CAL, DRAW_IMAGE_ENABLE);
            //DrawTextETC(400, 120, TEXT_ETC_TEMPUNIT, DRAW_IMAGE_ENABLE);
			if ((tempConfigData.calibrationConfig.TEMP_Span_Cal1/100)>=10) {

                tempIntVal = tempConfigData.calibrationConfig.TEMP_Span_Cal1;
                if (tempIntVal < 0)
                    tempIntVal = tempIntVal * (-1);
                if (compareSignValue < 0)
                    sprintf(strBuffer, "-%d.%01d", tempIntVal / 100, (tempIntVal % 100)/10);
                else
                    sprintf(strBuffer, " %d.%01d", tempConfigData.calibrationConfig.TEMP_Span_Cal1 / 100, (tempConfigData.calibrationConfig.TEMP_Span_Cal1 % 100)/10);
                //DrawMediumNumber(260, 120, strBuffer, YELLOW);
            	//DrawSmallNumber(210, 175, strBuffer, YELLOW);
            	DrawMediumNumber(182, 175, strBuffer, YELLOW);
                
				sprintf(strBuffer, "%01d", (tempConfigData.calibrationConfig.TEMP_Span_Cal1 % 100)/10);
                //DrawMediumNumber(260 + (22 * 3), 120, strBuffer, BROWN);
            	//DrawSmallNumber(210+ 12*3, 175, strBuffer, BROWN);
            	DrawMediumNumber(182+18*4, 175, strBuffer, BROWN);
			}
                ...
                //DrawMediumNumber(260 + (22 * 3), 120, strBuffer, BROWN);
            	//DrawSmallNumber(210+ 12*3, 175, strBuffer, BROWN);
            	DrawMediumNumber(200+18*4, 175, strBuffer, BROWN);
			}
            break;
#endif
...
#ifdef IMSI_PPM_MODIFY		
		if (currentData.S1PPM == 55) currentData.S1PPM=57;
		else if (currentData.S1PPM == 56) currentData.S1PPM=57;
		else if (currentData.S1PPM == 57) currentData.S1PPM=58;
		else if (currentData.S1PPM == 63) currentData.S1PPM=62;
		else if (currentData.S1PPM == 64) currentData.S1PPM=63;
		else if (currentData.S1PPM == 65) currentData.S1PPM=63;
		else if (currentData.S1PPM == 93) currentData.S1PPM=95;
		else if (currentData.S1PPM == 94) currentData.S1PPM=95;
		else if (currentData.S1PPM == 95) currentData.S1PPM=96;
		else if (currentData.S1PPM == 106) currentData.S1PPM=105;
		else if (currentData.S1PPM == 107) currentData.S1PPM=105;
#endif
...
//            sprintf(strBuffer, " %1d.%02d ", currentData.S1PPM / 100, currentData.S1PPM % 100);
//		}
       	DrawXLargeNumber(L_NUMBER1_X+27, L_NUMBER1_Y, strBuffer);
		display_mgl_1();

		

		if (Relay1_run_on_flag == 0){
	        // 온도 표시
			TFT_Fill(200, 154, 200+84, 154+5, BACK_COLOR3);
			TFT_Fill(200, 175, 200+84, 175+5, BACK_COLOR3);
	        if ((currentData.temperature) >= 9999) sprintf(strBuffer, " 99.9C ");	
	        else if ((currentData.temperature) <= -9999) sprintf(strBuffer, "-99.9C ");	
	        else if ((currentData.temperature) >= 0) 	sprintf(strBuffer, " %02d.%01dC ",      currentData.temperature / 100, (currentData.temperature % 100)/10); // C is 'C
	        else 										sprintf(strBuffer, "-%02d.%01dC ", (abs(currentData.temperature)) / 100, ((abs(currentData.temperature)) % 100)/10); // C is 'C
	...
	//        else sprintf(strBuffer, "%02d.%01dC", currentData.temperature / 100, currentData.temperature % 100); // C is 'C
	        ...
	        DrawSmallNumber(200, 159, strBuffer, BACK_COLOR3);
...
//            sprintf(strBuffer, "%2d.%04d", (currentData.S2PPM*10+imsi) / 10000, (currentData.S2PPM*10+imsi) % 10000);
        ...
        DrawXLargeNumber(L_NUMBER2_X-3, L_NUMBER2_Y, strBuffer);
		display_NTU_1();
...
//		}
	...
	//        else sprintf(strBuffer, "%02d.%01dC", currentData.temperature / 100, currentData.temperature % 100); // C is 'C
	        ...
	        if ((currentData.temperature) >= 9999) sprintf(strBuffer, " 99.9 ");	
	        else if ((currentData.temperature) <= -9999) sprintf(strBuffer, "-99.9 ");	
	        else if ((currentData.temperature) >= 0) 	sprintf(strBuffer, " %02d.%01d ",      currentData.temperature / 100, (currentData.temperature % 100)/10); // C is 'C
	        else 										sprintf(strBuffer, "-%02d.%01d ", (abs(currentData.temperature)) / 100, ((abs(currentData.temperature)) % 100)/10); // C is 'C
...
//            sprintf(strBuffer, "%2d.%04d", (currentData.S2PPM*10+imsi) / 10000, (currentData.S2PPM*10+imsi) % 10000);
       	...
       	DrawXLargeNumber(L_NUMBER1_X-30, L_NUMBER1_Y, strBuffer);
		display_uscm();
	...
	//        else sprintf(strBuffer, "%02d.%01dC", currentData.temperature / 100, currentData.temperature % 100); // C is 'C
	        ...
	        if ((currentData.temperature1) >= 9999) sprintf(strBuffer, " 99.9 ");	
	        else if ((currentData.temperature1) <= -9999) sprintf(strBuffer, "-99.9 ");	
	        else if ((currentData.temperature1) >= 0) 	sprintf(strBuffer, " %02d.%01d ",      currentData.temperature1 / 100, (currentData.temperature1 % 100)/10); // C is 'C
	        else 										sprintf(strBuffer, "-%02d.%01d ", (abs(currentData.temperature1)) / 100, ((abs(currentData.temperature1)) % 100)/10); // C is 'C
