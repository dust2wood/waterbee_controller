...
char  ph_temp=0;	// ph 또는 temp 선택하는거, 표시할때 잘못되는경우가 있어서 이를 사용함...
...
//#endif
            ...
            break;
        case 1:		// 485전송하는거, 한번쉬었다가 두번쨰에, 첫째 바이트를 전송하고 com485state=2로 한다.
            ...
            break;
        case 2:	// INT 에서 전체전송하기를 기다린다., INT 에서 전체전송하면 com485state=3이 된다.
            break;
        case 3:	// 전체 전송이끝나면 한번쉬었다가 두번째에 485전송을 끝낸다.
            if (flag10ms & FLAG10MS_RS485) {
                RS485_DRIVE_LOW;
				init_count=0;
                flag10ms &= ~FLAG10MS_RS485;
