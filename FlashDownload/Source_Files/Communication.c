...
uint32_t rx1HandlerCount = 0, rx2HandlerCount = 0, rx3HandlerCount = 0, com485State = 0, rs485DriveCount = 0;
...
void SensorComHandler(void)
{
	if(rx1Size > 0)
	{
		...
		if(flag10ms & FLAG10MS_RX1)
		{
			...
			{				
				if(rx1Buffer[0] == '$')
				{
					if(currentData.Device_Selector_Mode&SENSOR_1_MODE)	{
				  	...
				  	{
						if(state == 0 ) {
                   DrawTextsize120(40, 70, TEXT120_MEASURING, DRAW_IMAGE_ENABLE);
             }
						 else { 
           ...
           Couter_4++;								
				...
				else if (currentData.Device_Selector_Mode&SENSOR_2_MODE)
		    {
				  	...
				  	{
						   if(state == 0 ) {
                   DrawTextsize120(20, 70, TEXT120_MEASURING, DRAW_IMAGE_ENABLE);
               }
						   else { 
	 ...
	 ComCouter2++;
			  ...
			  if(ComCouter2 > 800 && state == 0 )
				{
             if( ComCouter_Prev == ComCouter)
	           	{   
								if(currentData.Device_Selector_Mode&SENSOR_1_MODE){
                    DrawTextsize120(40, 70, TEXT120_MEASURING, DRAW_IMAGE_DISABLE);
								    currentData.S1PPM = 0;
								    ...
								    RedrawValue();
								     ...
								     DrawTextsize120(20, 70, TEXT120_MEASURING, DRAW_IMAGE_DISABLE);
								    currentData.S2PPM = 0;
								    currentData.temperature = 0;
								    currentData.S2mV = 0;
									   trans_anlog_pwmVaule = 0;
...
void S1PPm_Filter_OUT_function (void)
{
	if(configData.adjustConfig.filterS1 != 0)
	{
	  ...
	  if(conter_Filter < configData.adjustConfig.filterS1){
        ...
        else if (currentData.Device_Selector_Mode&SENSOR_2_MODE)	{Filter_Data_Value[0][conter_Filter] = currentData.comm_S2NTU; }		
        ...
        conter_Filter =0; 
          ...
          else if (currentData.Device_Selector_Mode&SENSOR_2_MODE)	{ currentData.filterout_S2PPM   = (uint32_t)filterSumData;}					
...
{	 
	  ...
	  Adj_gradientS1 = (currentData.comm_S1PPM*0.01) * (configData.adjustConfig.gradientS1*0.01);
      ...
      Adj_gradientS2 = (currentData.comm_S1PPM*0.01) * (configData.adjustConfig.gradientS1*0.01);
		...
		if(first_flag == 0  && configData.adjustConfig.filter_update_flag != 0) {currentData.S1PPM =(uint32_t)((Adj_gradientS2 + Adj_offsetS2)*100);	}
    else currentData.S1PPM =(uint32_t)((Adj_gradientS1 + Adj_offsetS1)*100);		
		
	  if(couter_vaule_ppm < 60) {		
       Mesure_Data_Value[0][couter_vaule_ppm] = currentData.S1PPM;
       Mesure_Data_Value[3][couter_vaule_ppm] = currentData.temperature;
		 couter_vaule_ppm++;
		 if(couter_vaule_ppm == 60)
		 {
			  ...
			  for(i = 0; i < 60 ; i++)
			   {
	             ...
	             TEMP_card_Data[i]  = Mesure_Data_Value[3][i];
	   			}
				 WirteSDCard_flag = 1;
			}
	  		couter_vaule_ppm = 0;
 ...
 if(configData.adjustConfig.filter_update_flag == 0) {
	  Adj_gradientS1 = (currentData.comm_S2NTU*0.001) * (configData.adjustConfig.gradientS1*0.01);   
	  Adj_offsetS1 = (configData.adjustConfig.offsetS1*0.01);
    }
    else {
			       if(first_flag == 1) {
               Adj_gradientS1 = (currentData.comm_S2NTU*0.001) * (configData.adjustConfig.gradientS1*0.01);  
	             Adj_offsetS1 = (configData.adjustConfig.offsetS1*0.01);
							      ...
							      Adj_gradientS2 = (currentData.comm_S2NTU*0.01) * (configData.adjustConfig.gradientS1*0.01);
	                  Adj_offsetS2 = (configData.adjustConfig.offsetS1*0.01);
						 }
    }    	
     		if(first_flag == 0  && configData.adjustConfig.filter_update_flag != 0) {currentData.S2PPM =(uint32_t)((Adj_gradientS2 + Adj_offsetS2)*100);	}
        else   currentData.S2PPM =(uint32_t)((Adj_gradientS1 + Adj_offsetS1)*1000);
		
	  if(couter_vaule_ppm < 60) {		
       Mesure_Data_Value[0][couter_vaule_ppm] = currentData.S2PPM;
	   Mesure_Data_Value[3][couter_vaule_ppm] = currentData.temperature;
			 couter_vaule_ppm++;
			 if(couter_vaule_ppm == 60)
			 {
				  ...
				  for(i = 0; i < 60 ; i++)
				   {
             PPmSD_card_Data[i] = Mesure_Data_Value[0][i];
             TEMP_card_Data[i]  = Mesure_Data_Value[0][i];
           }
					 WirteSDCard_flag = 1;
				 }
          couter_vaule_ppm = 0;
...
void SendCalData(uint32_t vaule)
{	 
	 if(currentData.Device_Selector_Mode&SENSOR_1_MODE)	{
	  sprintf(strBuffer2, "%3d", vaule);
		tx1Buffer[0] = '$';
		tx1Buffer[1] = 'S';
		tx1Buffer[2] = 'E';
		tx1Buffer[3] = 'T';
...
void SendCalDataCurrent(uint32_t vaule)  
{	
		sprintf(strBuffer2, "%4d", vaule);
		tx1Buffer[0] = '$';
		tx1Buffer[1] = 'S';
		tx1Buffer[2] = 'E';
		tx1Buffer[3] = 'T';
...
void SendCalZeroData(uint32_t vaule)
{	 
	 if(currentData.Device_Selector_Mode&SENSOR_1_MODE)	{
	  sprintf(strBuffer2, "%3d", vaule);
		tx1Buffer[0] = '$';
...
void SendCalZeroDataCurrent(uint32_t vaule) 
{	
		sprintf(strBuffer2, "%4d", vaule);
		tx1Buffer[0] = '$';
		tx1Buffer[1] = 'S';
...
void SendCalDataTemp(uint32_t vaule)
{	
		sprintf(strBuffer2, "%4d", vaule);
		tx1Buffer[0] = '$';
		tx1Buffer[1] = 'S';
