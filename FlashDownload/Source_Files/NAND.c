...
uint32_t FSMC_NAND_WriteSmallPage(uint8_t *pBuffer, NAND_ADDRESS Address, uint32_t NumPageToWrite)
{
	...
	while((NumPageToWrite != 0x00) && (addressstatus == NAND_VALID_ADDRESS) && (status == NAND_READY))
	{
		...
		*(volatile uint8_t *)(NAND_FLASH_START_ADDR | ADDR_AREA) = ADDR_2nd_CYCLE(ROW_ADDRESS);   
		...
		{
			imsi2=pBuffer[index+1];
			*(volatile uint8_t *)(NAND_FLASH_START_ADDR | DATA_AREA) = imsi2;

			imsi1=pBuffer[index];
			*(volatile uint8_t *)(NAND_FLASH_START_ADDR | DATA_AREA) = imsi1;
		}
...
uint32_t FSMC_NAND_ReadSmallPage(uint8_t *pBuffer, NAND_ADDRESS Address, uint32_t NumPageToRead)
{
	...
	while((NumPageToRead != 0x00) && (addressstatus == NAND_VALID_ADDRESS))
	{	   
...
*/
	...
	{
//		TFT_String(50,70,"Nand Flash is OK ", WHITE, BLACK);	
	}
	else
	{
//		TFT_String(50,70,"Nand Flash is error ", WHITE, BLACK);	
	}
}
const NANDTYPE nand_db[11] =
{	{		K9F1208U,		512,		512*32,		32,		4096,		16,		33,		9,		64,		4096-4-33,		0,	},
	{		K9F1G08U,		2048,		(2048*64),		64,		1024,		64,		20,		16,		8,		1024-8-20,		0,	},
	{		K9F2G08U,		2048,		(2048*64),		64,		2048,		64,		40,		17,		8,		2048-8-40,		0,	},
	{		K9F4G08U,		2048,		(2048*64),		64,		4096,		64,		70,		18,		8,		4096-8-70,		0,	},
